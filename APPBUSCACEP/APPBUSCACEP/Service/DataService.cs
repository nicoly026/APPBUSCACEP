...
namespace APPBUSCACEP.Service
{
    public class DataService
    {
        ...
        {
            List<Logradouro> arr_lougradouros = new List<Logradouro>();

            using (HttpClient client = new HttpClient())
            {
                HttpResponseMessage response = await client.GetAsync("https://cep.metoda.com.br/lougradouro/by-bairro?id_cidade=4874&bairro=Jardim" + id_cidade + "&bairro" + bairro);
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    arr_lougradouros = JsonConvert.DeserializeObject<List<Logradouro>>(json);
                }
                else
                    throw new Exception(response.RequestMessage.Content.ToString());
            }
